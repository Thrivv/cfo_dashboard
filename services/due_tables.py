...
def generate_due_tables():
    ...
    if "Amount (AED)" in ar_df.columns:
        ar_df["Amount (AED)"] = pd.to_numeric(ar_df["Amount (AED)"], errors='coerce').fillna(0)
    if "Amount (AED)" in ap_df.columns:
        ap_df["Amount (AED)"] = pd.to_numeric(ap_df["Amount (AED)"], errors='coerce').fillna(0)
...
def get_AR_risk_data(ar_df):
    ...
    ar["Amount (AED)"] = pd.to_numeric(ar.get("Amount (AED)"), errors='coerce').fillna(0)
    ...
    risk_counts = ar["Risk"].value_counts().reindex(["High", "Medium", "Low"], fill_value=0).reset_index()
...
def get_AP_risk_data(ap_df):
    ...
    ap["Amount (AED)"] = pd.to_numeric(ap.get("Amount (AED)"), errors='coerce').fillna(0)
    ...
    risk_counts = ap["Risk"].value_counts().reindex(["High", "Medium", "Low"], fill_value=0).reset_index()
    risk_counts.columns = ['Risk', 'Count']
...
def view_risk_invoices(high_risk_invoices):
    ...
    if 'Overdue Days' in df.columns:
        ...
        if 'Due Date' in df.columns:
            df['Overdue Days'] = (today - pd.to_datetime(df['Due Date'], errors='coerce')).dt.days

            display_columns.append('Overdue Days')

    # Format Due Date and Amount for display
    df['Due Date'] = pd.to_datetime(df.get('Due Date'), errors='coerce').dt.date
    df['Amount (AED)'] = pd.to_numeric(df.get('Amount (AED)'), errors='coerce').fillna(0).round(2)
