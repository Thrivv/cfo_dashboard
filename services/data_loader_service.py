...
class DataLoaderService:
    ...
    def __init__(self):
        ...
        self._data_path = "data/static_kpis/cfo_dashboard_data.csv"
    ...
    def load_data(self) -> bool:
        ...
        try:
            ...
            print(f"Data loaded successfully: {self._raw_data.shape[0]} records, {self._raw_data.shape[1]} columns")
            return True
    ...
    def _process_data(self):
        ...
        try:
            self._processed_data['Date'] = pd.to_datetime(self._processed_data['Date / Period'], format='%m/%d/%Y')
        except ValueError:
            try:
                self._processed_data['Date'] = pd.to_datetime(self._processed_data['Date / Period'], format='%d-%m-%y')
            except ValueError:
                self._processed_data['Date'] = pd.to_datetime(self._processed_data['Date / Period'])
        
        numeric_columns = [
            'Revenue (Actual)', 'Revenue (Budget / Forecast)', 'Cost of Goods Sold (COGS)',
            'Gross Profit', 'Operating Expenses (OPEX)', 'EBITDA', 'Net Income',
            'Cash Inflows', 'Cash Outflows', 'Net Cash Flow', 'Cash Balance',
            'Total Assets', 'Total Liabilities', 'Equity', 'Working Capital',
            'Current Ratio', 'Debt-to-Equity Ratio', 'Return on Equity (ROE)',
            'Return on Assets (ROA)', 'Inventory Value', 'Inventory Turnover'
        ]
        
        for col in numeric_columns:
            if col in self._processed_data.columns:
                self._processed_data[col] = pd.to_numeric(self._processed_data[col], errors='coerce')
    ...
    def get_data_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "business_units": self._raw_data['Business Unit / Department'].unique().tolist() if 'Business Unit / Department' in self._raw_data.columns else [],
...
def load_cfo_data() -> Optional[pd.DataFrame]:
    """
    Load CFO data for backward compatibility.
    
    Returns:
        pd.DataFrame or None: CFO data if available, None otherwise
    """
