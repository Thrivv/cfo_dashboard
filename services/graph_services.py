...
• IMPORTANT: If you use any formatting characters, the response will be rejected."""
        ...
        for attempt in range(max_retries):
            ...
            if _validate_llm_output(insights):
                ...
                insights = re.sub(
                    r"[`*_#]", "", insights
                )  # Remove any remaining formatting chars
                ...
                for line in lines:
                    ...
                    if line.startswith("Key Findings:"):
                        ...
                        key_findings_section.append(
                            f'<p style="font-weight: bold;">{line}</p>'
                        )
                        ...
                        key_findings_section.append(
                            f'<p style="margin-left: 20px;">{line}</p>'
                        )
...
def generate_forecast_insights(
    ...
    Returns:
        str: LLM-generated insights about the forecast.
    """
    return generate_llm_forecast_insights(
        forecast_data, department, start_date, end_date, max_retries=3
    )
...
def display_forecast_chart(
    ...
    if chart_type == "streamlit":
        ...
        fig = create_forecast_chart_with_plotly(
            forecast_data, department, chart_height, start_date, end_date
        )
        if fig:
