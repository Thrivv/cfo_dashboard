...
def _apply_plot_theme(fig: go.Figure, height: int = 400, title: str | None = None) -> go.Figure:
...
def render():
    ...
    if raw_df is not None and not raw_df.empty:
        # Page Header
        st.markdown('<div class="panel"><div class="section-title">Revenue Forecasting</div>', unsafe_allow_html=True)
        
        # Department selection for all charts
        departments = ['Sales', 'Marketing', 'IT', 'HR', 'Operations', 'Finance', 'All Departments']
        selected_dept = st.selectbox("Select Department for Analysis", departments, key="main_dept_selector")
            ...
            if selected_dept != 'All Departments':
                ...
                if not dept_data.empty:
                    ...
                    if len(dept_data) > 1:
                        ...
                        prev_spend = prev.get('Operating Expenses (OPEX)', 0) + prev.get('Cost of Goods Sold (COGS)', 0)
                        spend_trend = ((total_spend - prev_spend) / max(prev_spend, 1)) * 100
                    else:
                        spend_trend = 0
        ...
        with col2:
            ...
            if selected_dept != 'All Departments':
                ...
                fig = px.bar(dept_spend, x='Business Unit / Department', y='Total Spend', 
                            title='Total Spend by Department', color='Total Spend',
