...
def is_forecast_question(question):
    ...
    forecast_keywords = ['forecast', 'predict', 'projection', 'graph', 'chart']
...
def is_rag_question(question):
    ...
    rag_keywords = ['invoice', 'payment', 'overdue', 'regulation', 'requirement', 'license', 'warning', 'opportunity', 'account receivable', 'account payable', 'receivables', 'payables', "Purchase orders", 'terms and conditions', 'discount', 'penalty', 'late fee', 'retail payment', 'card scheme']
    question_lower = question.lower()
...
def render():
    ...
    with col_main:
        ...
        with st.form(key="question_form", clear_on_submit=True):
            user_question = st.text_area("Your question", height=72, placeholder="Ask about financial metrics, forecasts, invoices, regulations, or business performance...", key="ai_question_input", label_visibility="collapsed")
            
            col1, col2 = st.columns([1, 1])
            with col1:
                submitted = st.form_submit_button("Ask TO KraYa", type="primary", disabled=st.session_state.get('processing_question', False))
            with col2:
                clear_clicked = st.form_submit_button("Clear", disabled=st.session_state.get('processing_question', False))
    ...
    with col_side:
        with st.expander("Quick questions", expanded=True):
            ...
            for idx, question in enumerate(questions):
                ...
                if st.button(question, key=button_key, use_container_width=True, disabled=button_disabled):
        ...
        with st.expander("CFO Data Status", expanded=True):
            ...
            if st.session_state.ai_chat_history:
                st.metric("CFO queries processed", len(st.session_state.ai_chat_history))
