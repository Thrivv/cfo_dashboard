...
from components.data_filter import (
    apply_filters,
    parse_date_column,
    get_filter_summary,
    validate_filters,
    parse_quarterly_date,
)
            ...
            if df is not None and not df.empty and raw_df is not None:
                ...
                if st.session_state.get("show_alerts_only", False):
                    ...
                    filtered_df_for_alerts = apply_filters(
                        raw_df, st.session_state.cfo_filters
                    )
                    latest_raw_for_alerts = (
                        filtered_df_for_alerts.iloc[-1]
                        if not filtered_df_for_alerts.empty
                        else temp_latest_raw
                    )
                ...
                if not filtered_df.empty:
                    ...
                    st.warning(
                        f"No data available for selected filters: {st.session_state.cfo_filters['unit']}"
                    )
                ...
                if chart_key not in st.session_state:
                    ...
                    profit_trend = filtered_df[
                        ["Date / Period", "Gross Profit", "EBITDA", "Net Income"]
                    ].copy()
                    ...
                    if "Date" not in profit_trend.columns:
                        if st.session_state.cfo_filters.get("period") == "Quarterly":
                            ...
                            profit_trend["Date"] = profit_trend["Date / Period"].apply(
                                parse_quarterly_date
                            )
                        elif st.session_state.cfo_filters.get("period") == "Yearly":
                            # For yearly data, convert to year-end dates
                            profit_trend["Date"] = pd.to_datetime(
                                profit_trend["Date / Period"].astype(str) + "-12-31"
                            )
                        else:
                            # For monthly data, use existing date parsing
                            profit_trend["Date"] = parse_date_column(
                                profit_trend["Date / Period"]
                            )
                ...
                with col1:
                    ...
                    fig = _apply_plot_theme(
                        fig, height=300, title=f"AR Aging Analysis (DSO: {dso:.0f})"
                    )
                    st.plotly_chart(fig, use_container_width=True)
                ...
                with col2:
                    ...
                    fig = _apply_plot_theme(
                        fig, height=300, title=f"AP Analysis (DPO: {dpo:.0f})"
                    )
                    st.plotly_chart(fig, use_container_width=True)
                ...
                if show_detailed:
                    ...
                    with col1:
                        ...
                        if inventory_key not in st.session_state:
                            ...
                            if "Date" not in inventory_trend.columns:
                                if (
                                    st.session_state.cfo_filters.get("period")
                                    == "Quarterly"
                                ):
                                    inventory_trend["Date"] = inventory_trend[
                                        "Date / Period"
                                    ].apply(parse_quarterly_date)
                                elif (
                                    st.session_state.cfo_filters.get("period")
                                    == "Yearly"
                                ):
                                    inventory_trend["Date"] = pd.to_datetime(
                                        inventory_trend["Date / Period"].astype(str)
                                        + "-12-31"
                                    )
                                else:
                                    inventory_trend["Date"] = parse_date_column(
                                        inventory_trend["Date / Period"]
                                    )
                    ...
                    with col2:
                        ...
                        capex_key = (
                            f"capex_trend_{hash(str(st.session_state.cfo_filters))}"
                        )
                        if capex_key not in st.session_state:
                            ...
                            if "Date" not in capex_opex_trend.columns:
                                if (
                                    ...
                                    capex_opex_trend["Date"] = capex_opex_trend[
                                        "Date / Period"
                                    ].apply(parse_quarterly_date)
                                elif (
                                    st.session_state.cfo_filters.get("period")
                                    ...
                                    capex_opex_trend["Date"] = pd.to_datetime(
                                        capex_opex_trend["Date / Period"].astype(str)
                                        + "-12-31"
                                    )
                                else:
                                    capex_opex_trend["Date"] = parse_date_column(
                                        capex_opex_trend["Date / Period"]
                                    )
